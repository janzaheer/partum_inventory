...
class EanBarCode:
    ...
    def computeChecksum(self, arg):
        ...
        total = 0
        for i in range(12):
            total += int(arg[i]) * weight[i]
        z = (magic - (total % magic)) % magic
        if z < 0 or z >= magic:
            return None
        return z
    ...
    def verifyChecksum(self, bits):
        ...
        if codeBarChecksum != computedChecksum:
            raise Exception(
                "Bad checksum is %s and should be %s" % (codeBarChecksum, computedChecksum)
            )
    ...
    def getImage(self, value, height=50, thickness=1):
        ...
        for bit in range(len(bits)):
            if bits[bit] == '1':
                draw.rectangle(
                    ((thickness * bit + position, 0),
                     (thickness * bit + position + thickness, height - 10)),
                    fill=0,
                )
            elif bits[bit] == 'L':
                draw.rectangle(
                    ((thickness * bit + position, 0),
                     (thickness * bit + position + thickness, height - 3)),
                    fill=0,
                )
        return im
